...
var coins = 158, highScore = 0,score = 0;
...
function preload(){
    ...
    playerCharSP_IMG2 = loadImage("./assets/yellow.png")
...
function setup(){
    ...
    s1_home = new NewNavigate("./assets/orange.png");
...
function draw(){
    ...
    s1_home.display(name_p,isEditClick,coins,highScore,gameV);
    ...
    if(isSPclicked == true){
       ...
       // console.log("what val"+s)
        ...
        if((keyDown("space")||keyDown("w")||keyIsDown(UP_ARROW))&&fuelLeft>0&&isResting==false&&obs_dif_data[0].dif_l != 0&&exlevelData.length >2){  //here i added: &&obs_dif_data[0].dif_l != 0
            ...
            if(obs_dif_data[0].dif_l != 0&&exlevelData.length >2){
               playerCharSP.velocity.y = 8; 
            }
       ...
       // var f_v
        ...
        if((keyIsDown(LEFT_ARROW)||keyDown("a"))&&obs_dif_data[0].dif_l != 0){
        ...
        if((keyIsDown(RIGHT_ARROW)||keyDown("d"))&&obs_dif_data[0].dif_l != 0){
            if(platformGroup.overlap(playerCharSP)==false){
                if(which_l_play>=lastPlatformCollided){
                    playerCharSP.position.x += 7
                }else if(which_l_play<=lastPlatformCollided){
